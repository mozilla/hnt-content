...
describe('validateCrawlArticleMessage', () => {
  ...
  it('rejects a missing article_refresh_minutes', () => {
    expect(() =>
      validateCrawlArticleMessage(
        omit(VALID_ARTICLE, 'article_refresh_minutes'),
      ),
    ).toThrow(/article_refresh_minutes must be a finite number/);
  });

  it('rejects a non-numeric article_refresh_minutes', () => {
    expect(() =>
      validateCrawlArticleMessage({
        ...VALID_ARTICLE,
        article_refresh_minutes: '20',
      }),
    ).toThrow(/article_refresh_minutes/);
  });

  it.each([0, -5])(
    'rejects a non-positive article_refresh_minutes (%s)',
    (value) => {
      expect(() =>
        validateCrawlArticleMessage({
          ...VALID_ARTICLE,
          article_refresh_minutes: value,
        }),
      ).toThrow(/article_refresh_minutes must be a positive number/);
    },
  );
});
