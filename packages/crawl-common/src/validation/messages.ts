...
export function validateCrawlArticleMessage(raw: unknown): CrawlArticleMessage {
  ...
  const message: CrawlArticleMessage = {
    ...
    article_refresh_minutes: requireNumber(
      obj,
      'article_refresh_minutes',
      label,
    ),
  };
  // The worker gates re-fetch on this window, so a non-positive value
  // would make every article look perpetually stale.
  if (message.article_refresh_minutes <= 0) {
    throw new MessageValidationError(
      `${label}.article_refresh_minutes must be a positive number`,
    );
  }
...
/**
 * Validate a crawl-article-discovery message. Requires the page url
 * and at least one context. The page crawl cadence is a global worker
 * setting rather than a per-message field, so no interval is carried
 * here. Fields are checked in declaration order so the first error
