...
import {
  ...
  REDIS_PORT_DEFAULT,
  ZYTE_RATE_LIMIT_BURST_DEFAULT,
  ZYTE_RATE_LIMIT_MAX_WAIT_MS_DEFAULT,
...
// any changes to these two interfaces must also be represented in the
// `configSchema` below.
interface corpusApiConfig {
  audience: string;
  endpoint: string;
  issuer: string;
  jwkJson: string;
...
interface crawlWorkerConfig {
  ...
  articleDiscoveriesTopic: string;
  articleFetchTtlMinutes: number;
  articlesTopic: string;
  corpusApi: corpusApiConfig;
  crawlArticleDiscoverySubscription: string;
  crawlArticleSubscription: string;
  crawlArticleTopic: string;
  ...
  zyteApiKey: string;
  zyteRateLimitBurst: number;
  zyteRateLimitMaxWaitMs: number;
  zyteRateLimitPerMinute: number;
}

const configSchema: JSONSchemaType<crawlWorkerConfig> = {
  type: 'object',
  properties: {
    ackDeadlineSeconds: { type: 'number' },
    articleDiscoveriesTopic: { type: 'string' },
    articleFetchTtlMinutes: { type: 'number' },
    articlesTopic: { type: 'string' },
    corpusApi: {
      type: 'object',
      properties: {
      ...
      required: ['audience', 'endpoint', 'issuer', 'jwkJson'],
    },
    crawlArticleDiscoverySubscription: { type: 'string' },
    crawlArticleSubscription: { type: 'string' },
    crawlArticleTopic: { type: 'string' },
    lockTtlSeconds: { type: 'number' },
    maxExtensionSeconds: { type: 'number' },
    port: { type: 'number' },
    projectId: { type: 'string' },
    pubsubEmulatorHost: { type: 'string', nullable: true },
    pubsubMaxMessages: { type: 'integer' },
    redisHost: { type: 'string' },
    redisPort: { type: 'integer' },
    service: { type: 'string' },
    workerRole: { type: 'string' },
    zyteApiKey: { type: 'string' },
    zyteRateLimitBurst: { type: 'number' },
    zyteRateLimitMaxWaitMs: { type: 'number' },
    zyteRateLimitPerMinute: { type: 'number' },
  },
  required: [
    'ackDeadlineSeconds',
    'articleDiscoveriesTopic',
    'articleFetchTtlMinutes',
    'articlesTopic',
    'corpusApi',
...
const ackDeadlineSeconds = numberEnv(
  process.env.ACK_DEADLINE_SECONDS,
  ACK_DEADLINE_SECONDS_DEFAULT,
);

if (ackDeadlineSeconds <= ACK_DEADLINE_SECONDS_MIN) {
  throw new Error(
    `ACK_DEADLINE_SECONDS must be > ${ACK_DEADLINE_SECONDS_MIN}, got ${ackDeadlineSeconds}`,
  );
}

// Lock TTL must clear just before redelivery, so it tracks the ack
// deadline rather than the longer max extension.
const lockTtlSeconds = ackDeadlineSeconds - LOCK_TTL_ACK_SECONDS_DELTA;
...
const pubsubMaxMessages = numberEnv(
  process.env.PUBSUB_MAX_MESSAGES,
  PUBSUB_MAX_MESSAGES_DEFAULT,
);
...
const config = {
  ...
  articleFetchTtlMinutes: numberEnv(
    process.env.ARTICLE_FETCH_TTL_MINUTES,
    ARTICLE_FETCH_TTL_MINUTES_DEFAULT,
  ),
  ...
  lockTtlSeconds,
  maxExtensionSeconds,
  port: numberEnv(process.env.PORT, APP_PORT_DEFAULT),
  ...
  redisPort: numberEnv(process.env.REDIS_PORT, REDIS_PORT_DEFAULT),
  ...
  zyteRateLimitBurst: numberEnv(
    process.env.ZYTE_RATE_LIMIT_BURST,
    ZYTE_RATE_LIMIT_BURST_DEFAULT,
  ),
...
let validConfig: crawlWorkerConfig;

// create a validator based on the schema defined above
const configValidator = ajv.compile(configSchema);

// validate the config - this is primarily testing that the minimal ENV vars
// are present. without them, the app cannot function.
if (configValidator(config)) {
  validConfig = config;
} else {
  throw new Error(`crawl worker config validation failed`, {
    cause: JSON.stringify(configValidator.errors),
  });
}
